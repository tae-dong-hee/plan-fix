...
같은 권한의 최신 링크가 아직 유효하면 그 링크를 다시 반환한다. 생성일·만료일은 연장하지 않는다.
반복 준비나 동시 요청만으로 링크가 늘어나지 않으며 카카오 공유·링크 복사는 같은 토큰을 사용한다.
권한이 달라지거나 최신 링크가 만료/취소되어 사용할 수 없으면 새 링크를 발급한다.
편집 → 읽기 → 편집 순서로 준비하면 마지막 편집은 과거 편집 링크를 재사용하지 않는다.
소유자의 직접 권한 변경 또는 멤버 제거 기준 때문에 최신 링크로 요청한 권한을 적용할 수 없으면,
같은 권한 요청도 새 링크를 발급한다. 새 링크를 준비하는 것만으로 기존 멤버의 권한을 바꾸지는 않는다.

이미 공유한 과거 링크는 유효기간과 초대 취소 정책을 그대로 따른다.
멤버 관리에서는 발급 이력의 개수와 관계없이 편집·읽기 초대를 각각 하나의 관리 항목으로 표시한다.
다시 공유는 위 준비 API를 호출해 최신 발급 순서를 적용한다. 초대 취소는 해당 권한의 기존 링크 전체를
함께 무효화한다. 이미 참여한 멤버의 현재 권한은 그대로 유지한다.
...
- `GET /api/v1/courses/{courseId}/invite-groups` — 소유자만 조회하며 `Cache-Control: no-store`를 반환한다.
- `DELETE /api/v1/courses/{courseId}/invite-groups/{role}` — 소유자만 해당 권한의 모든 원본 링크를 취소한다.

목록은 현재 유효한 링크가 있는 권한마다 한 항목이다. `role`은 `EDITOR` 또는 `VIEWER`이며,
`createdAt`·`expiresAt`은 그 권한에서 가장 최근에 발급된 유효 링크의 시각이다.
토큰 목록이나 과거 중복 개수는 노출하지 않는다. 나만 보기 코스는 빈 배열을 반환한다.

```json
[{ "role": "EDITOR", "createdAt": "2026-09-21T01:03:18Z", "expiresAt": "2026-09-28T01:03:18Z" }]
```

그룹 조회는 저장된 링크나 멤버를 수정하지 않는다. 이전에 전송한 링크도 원래의 발급 ID·권한·만료일로
수락하므로 최신 권한이나 소유자의 직접 변경·제거 정책을 되돌리지 못한다. 다시 공유는 대표 토큰을
그대로 재전송하는 대신 준비 API를 호출한다. 편집 → 읽기 → 편집 재공유라면 새 편집 링크를 발급할 수 있다.

취소는 코스 잠금 안에서 같은 권한의 유효·만료 링크와 카카오 전송 확인을 모두 삭제한다.
다른 권한·다른 코스의 링크와 기존 멤버 권한은 유지한다. 이미 취소된 그룹의 반복 취소도 `204`다.
조회·취소는 소유자 전용이고 `OWNER` 등 초대할 수 없는 권한은 거부한다.
구버전 화면을 위한 원본 `GET /invites`, `DELETE /invites/{token}`의 개별 토큰 계약은 유지한다.
