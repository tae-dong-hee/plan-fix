...
`2026-09-19-add-password-reset-tokens.sql`은 계정별 재설정 토큰 해시와 세션 버전을 보관한다.
새 백엔드의 로그인·인증도 이 테이블을 조회하므로 백엔드 배포 전에 적용한다. 토큰 원문은 저장하지
않으며, 사용한 토큰을 지울 때 행과 세션 버전은 유지한다. 행을 삭제하면 이전 로그인 토큰의
무효화 정보가 사라지므로 만료 토큰 청소 작업에서 이 테이블의 행을 삭제하지 않는다.
SMTP 설정과 기능 확인 절차는 [비밀번호 재설정 가이드](../password-reset.md)를 참고한다.
...
## 누락 장소 사진 3곳 보완 (2026-09-19)

`2026-09-19-backfill-remaining-spot-photos.sql`은 산계리3층석탑(527), 강릉통일공원안보전시관(534), 보광리분청자요지(544)의 빈 대표사진만 채운다. 프런트의 정적 파일과 출처 카탈로그를 먼저 배포하고 대상527·534·544와 원본420의 장소·관광정보·갤러리를 백업한 뒤 적용한다. 신원이나 사진 상태가 바뀌면 전체 롤백하며 원본420의 사진은 변경하지 않는다. [검수 근거](../assets/remaining-spot-photos.md)를 확인한다.
