...
아이디를 잊었다면 가입 이메일로 아이디 안내 메일을 받을 수 있다.
전체 연결은 [계정 복구 안내](account-recovery.md)를 참고한다.
...
1. [비밀번호 재설정 마이그레이션](migrations/2026-09-19-add-password-reset-tokens.sql)과
   [복구 요청 제한 마이그레이션](migrations/2026-09-19-add-account-recovery-rate-limits.sql)을 DB에 적용한다.
