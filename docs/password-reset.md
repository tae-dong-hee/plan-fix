...
등록된 휴대폰이 있다면 문자 인증으로 아이디를 확인하고 바로 비밀번호를 재설정할 수 있다.
전체 연결과 문자 설정은 [계정 복구 안내](account-recovery.md)를 참고한다.
...
| `MAIL_SMTP_SSL_ENABLE` | 암시적 TLS 사용, 기본 `false`. 465 포트는 제공자 지침에 따라 설정 |
...
두 API는 로그인 없이 접근하며 성공 시 `204 No Content`를 반환한다. 메일 요청은 일치하는
활성 자체 로그인 계정인지 먼저 확인한다. 불일치는 `400 RECOVERY_ACCOUNT_MISMATCH`와
“아이디 또는 이메일이 일치하지 않습니다.”를 반환한다. 이 제품 요구사항에 따라 입력 정보의
일치 여부가 노출되므로 발송 전 요청 횟수도 제한한다. 같은 계정의 60초 내 재요청은 `429`이다.
설정 누락 또는 SMTP 발송 실패는 `503`이고, SMTP 서버가 메일을 수락한 경우에만 `204`이다.
...
메일 요청의 성공은 SMTP 접수를 의미하며 수신함 도착까지 보장하지 않는다. SMTP 발송 실패는
화면에 표시하고 서버 로그에서 원인을 확인하여 설정을 수정한 뒤 60초 후 다시 요청한다.
로그에 재설정 토큰, 메일 본문, 수신 주소, 공급자의 원문 오류를 남기지 않는다.

## 배포와 확인

1. [비밀번호 재설정 마이그레이션](migrations/2026-09-19-add-password-reset-tokens.sql)과
   [휴대폰 인증·요청 제한 마이그레이션](migrations/2026-09-19-add-phone-recovery.sql)을 DB에 적용한다.
