                    ...
                    <div key={item.commentId} className={depth > 0 ? "ml-6 border-l-2 border-primary/20 pl-4" : ""}>
                      <div className="rounded-xl bg-muted/40 px-4 py-3 text-sm">
                        ...
                        <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
                          {depth === 0 && <button type="button" disabled={editingCommentId !== null} onClick={() => { setReplyToId(item.commentId); setReplyText(""); }} className="inline-flex items-center gap-1 hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"><Reply className="h-3.5 w-3.5" />대댓글</button>}
                          {currentUserId === item.userId && editingCommentId !== item.commentId && <button type="button" onClick={() => { setEditingCommentId(item.commentId); setEditingText(item.content); setReplyToId(null); setReplyText(""); }} className="inline-flex items-center gap-1 hover:text-primary"><Pencil className="h-3.5 w-3.5" />수정</button>}
                        ...
                        {depth === 0 && replyToId === item.commentId && editingCommentId === null && <div className="mt-3 flex gap-2"><input value={replyText} onChange={(event) => setReplyText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void submitReply(item.commentId); }} placeholder="대댓글을 남겨보세요" maxLength={1000} className="min-w-0 flex-1 rounded-lg border border-border bg-background px-3 py-2 text-sm" autoFocus /><button type="button" onClick={() => void submitReply(item.commentId)} disabled={!replyText.trim() || commentActionId !== null} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-50">등록</button></div>}
                      </div>
