...
function setup({ html = "<p></p>", initialFiles = [] as File[] } = {}) {
  ...
  function Harness({ title = "강릉 여행" }: { title?: string }) {
    const [files, setFiles] = useState(initialFiles);
    return <StoryWritingAssistant title={title} files={files} editorRef={editorRef} onBusyChange={onBusyChange}
  ...
  return {
    ...
    changeTitle(title: string) {
      rendered.rerender(<Harness title={title} />);
    },
...
describe("StoryWritingAssistant", () => {
  test("여러 사진을 선택하면 제목과 기억을 보내고 빈 본문에 자동으로 초안을 채운다", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/강릉 안목해변/), { target: { value: "친구와 함께 보낸 오후" } });
  ...
  test("생성 중에는 사진 변경을 막고 직접 작성으로 취소한 뒤에만 새 요청을 시작한다", async () => {
    ...
    expect(screen.getByPlaceholderText(/강릉 안목해변/)).toBeDisabled();
